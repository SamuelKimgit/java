...
public class CalendarEx04Main implements LoggerManager {
	...
	public static void main(String[] args) {
		...
		// '월'은 0~11까지의 값을 가지므로 1을 빼주어야 한다.
		sDay.set(year, month-1, 1); // 2022 1 1 / 2022 - 02 - 01
		eDay.set(year, month, 1); // 2022 2 1 / 2022 - 03 - 01
		//LOG.debug(toString(sDay));
		
		// 2022-03-01 -1 => 그 달의 마지막 일
		eDay.add(Calendar.DATE, -1);
		
		//LOG.debug(toString(sDay)); // 2022년2월1일
		//LOG.debug(toString(eDay)); // 2022년2월28일
		
		// 시작요일
		START_DAY_OF_WEEK = sDay.get(Calendar.DAY_OF_WEEK);
		//LOG.debug("시작요일: " + START_DAY_OF_WEEK); // 0(일요일)
		
		// '월'의 마지막일
		END_DAY = eDay.get(Calendar.DATE);
		//LOG.debug("'월'의 마지막일: " + END_DAY);
		...
		for(int i = 1,n = START_DAY_OF_WEEK; i <= END_DAY; i++, n++) {
			...
			// DAY_OF_WEEK가 토요일(7)이면 Enter("\n")
			if(n % 7 == 0) {
				System.out.print("\n");
			}
			//System.out.print(n); n 확인용
