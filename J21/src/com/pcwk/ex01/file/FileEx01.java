...
//		"C:\\DCOM_20220127\\01_JAVA\\workspace\\J21\\src\\com\\pcwk\\ex01\\file\\FileEx01.java"
	...
	public static void printFileList(File dir) {
		...
		int dirNum = subDir.size(); // subDir의 갯수
		
		// files.length(파일+디렉토리) - 디렉토리 = 파일 수 
		int fileNum = files.length - dirNum; // 현재 파일 수 
		
		// 총 파일 누적
		// 총 디렉토리 누적
		
		totalFiles += fileNum; // 총 파일 누적
		...
		for(int i = 0; i < subDir.size(); i++) {
			int idx = Integer.parseInt(subDir.get(i)); // String type -> int type변환 parse
